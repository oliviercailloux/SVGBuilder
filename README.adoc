...
== Todo
Should make Displacement an interface, use the record only as an internal implementation of it; and introduce a Size class that is a positive displacement. Or make it not a displacement? Might serve for scale and spacing concepts (width, height).

Size + displacement equals size
displacement + size equals displacement

A Size is a Zone that starts at zero.

Can zone size be negative; can setSize over elements move starting point…?
...
- A Displacement may be positive or negative (meaning that each of its coordinate can be). It should be used when planning to add it to a Point or another Displacement. It can represent a size as well (which are considered possibly negative, for the same reason that a duration can be negative).
- A Zone delimits a rectangular area in the coordinate system. It is caracterized by two Points, the top-left and the bottom-right corners, or equivalently, a Point and a positive Displacement. It corresponds to a localized instance of a vector.
...
== Rationale
This library separates points from displacements (instead of embracing a general vector concept) because is not a general purpose or efficiency focused mathematical library. It is rather meant for use in high level drawing operations. This is less flexible but offers clearer semantics. 
