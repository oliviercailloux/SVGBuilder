...
== Geometry
For (zero, zero)-based coordinate systems which cannot go negative, with y coordinate going “down”, such as screen coordinates.

This library distinguishes points from displacements and has no generic vector concept. A point is to a displacement what an instant is to a duration.

- A Point is a positive absolute (x, y) position in the coordinate system.
- A Displacement may be positive or negative (meaning that each of its coordinate can be). It should be used when planning to add it to a Point or another Displacement.
- A Size is positive, it has a width and a height. It can also represent a scale or a spacing.
- A Zone delimits a rectangular area in the coordinate system. It is caracterized by two Points, the top-left and the bottom-right corners, or equivalently, a Point and a Size.

A Size can be converted to a Displacement, which can be added to a Point, several Point instances determine a Zone, which yields a Size.

When you want a vector, use a Displacement if you intend to use it for computing coordinates; transform it to a Point or a Zone once you want it to represent an absolute location or rectangle; from the Zone obtain a Size when you want to represent a (positive) size.
...
== Rationale
This library separates points from displacements (instead of embracing a general vector concept) because it is meant for use in high level drawing operations (rather than being a general purpose or efficiency focused mathematical library). This is less flexible but offers clearer semantics. 

Some related discussion https://gamedev.stackexchange.com/questions/44314/do-i-need-a-point-and-a-vector-object-or-just-using-a-vector-object-to-represen[here]

== About SVG 2
Todo: Implement SVG1 DOM and https://svgwg.org/svg2-draft/types.html#InterfaceSVGElement[SVG2 DOM] and in particular getBBox.
